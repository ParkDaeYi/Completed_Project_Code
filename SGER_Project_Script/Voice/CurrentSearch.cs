...
public class CurrentSearch : MonoBehaviour
{
    /*
* date 2018.08.16
* author INHO
...
*/
    ...
    public Dictionary<int, Dictionary<string, GameObject>> _fileTable;
    public int _key;
    public int _prev_key;
    ...
    void Update()
    {
        ...
        {
            if (_allSearch._inputField.text != "") _allSearch._inputField.text = "";
            _cameraMoveAroundSwi._cameraAroun = false;
            _isReady = true;
        }
        if (_isReady && (Input.GetKeyDown(KeyCode.Return) || Input.GetKeyDown(KeyCode.KeypadEnter)))
        {
            if (_allSearch._all_content.gameObject.activeSelf) _allSearch._all_content.gameObject.SetActive(false);
            _startDBController._pathText.text = "Search Results";
            _scrollRect.content = _current_content.GetComponent<RectTransform>();
            _current_content.gameObject.SetActive(true);
            ...
            {
                string str = _content.GetChild(i).name;
                if (str.Equals("VoiceButton")) continue;
                root.insert(str + '\0', 0);
            }

            foreach (string str in syn)
            {
                if (_fileTable[_prev_key].ContainsKey(str))
                    _fileTable[_prev_key][str].SetActive(false);
            }
            syn.Clear();
            ...
            {
                Debug.Log(str);
                if (_fileTable[_key].ContainsKey(str))
                    _fileTable[_key][str].SetActive(true);
            }          
            root.clear();
            _prev_key = _key;
        }
